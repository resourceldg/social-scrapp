...
FACEBOOK_SELECTORS = {
    "result_links": ["a[href*='facebook.com']", "a[role='link']"],
}
...
class FacebookScraper:
    ...
    def scrape(self, driver: WebDriver, config: AppConfig) -> list[Lead]:
        ...
        for keyword in config.facebook_keywords:
            try:
                url = f"{self.base_url}/search/pages/?q={quote(keyword)}"
                logger.info("Facebook keyword=%s", keyword)
                driver.get(url)
                WebDriverWait(driver, 20).until(EC.presence_of_element_located((By.TAG_NAME, "body")))
                random_delay(config.min_delay, config.max_delay)
                driver.execute_script("window.scrollTo(0, document.body.scrollHeight * 0.3);")
                random_delay(config.min_delay, config.max_delay)
                save_debug_html(driver, config.debug_html_dir, f"facebook_{quote(keyword)}.html", config.save_debug_html)

                soup = soup_from_html(driver.page_source)
                text = soup.get_text(" ", strip=True)
                links = []
                for selector in FACEBOOK_SELECTORS["result_links"]:
    ...
    def _lead_from_candidate(self, keyword: str, url: str, text: str) -> Lead:
        ...
        return Lead(
            ...
            country=country,
            bio=text[:500],
            category="facebook_page_or_group",
